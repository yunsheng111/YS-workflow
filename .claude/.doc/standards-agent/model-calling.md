...
1. **禁止省略 `--backend` 参数** -- codeagent-wrapper 默认使用 Codex 后端，省略将导致 frontend-agent 错误地调用 Codex
2. **禁止依赖默认行为** -- 即使 backend-agent 的默认后端恰好是 Codex，也必须显式指定 `--backend codex`（消除架构脆弱性）
3. **ROLE_FILE 必须匹配模型** -- Gemini 代理使用 `gemini/` 下的角色，Codex 代理使用 `codex/` 下的角色
...
ROLE_FILE: .ccg/prompts/gemini/<role>.md
<TASK>
需求：<增强后的需求>
上下文：<项目上下文>
</TASK>
...
ROLE_FILE: .ccg/prompts/codex/analyzer.md
<TASK>
需求：<增强后的需求>
上下文：<项目上下文>
</TASK>
...
```bash
{{CCG_BIN}} --backend codex {{LITE_MODE_FLAG}} resume <CODEX_SESSION> - "{{WORKDIR}}" <<'EOF'
ROLE_FILE: .ccg/prompts/codex/<role>.md
<TASK>
需求：<增强后的需求>
上下文：<项目上下文>
</TASK>
...
single-model 代理降级时**不切换到另一个模型**，而是由 Claude 独立完成：
...
`config.toml` 中的 `[routing.<command>]` 配置声明了命令的主导模型：
...
运行时可通过 `command-renderer.cjs` 的 `getRoutingBackend()` 函数读取此配置，并自动渲染为 `{{BACKEND_FLAG}}` 占位符。代理文件中的显式 `--backend` 模板与此配置保持一致。
