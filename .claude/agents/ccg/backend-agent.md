...
调用 Codex 进行后端技术可行性分析。**必须使用以下显式命令模板**（禁止省略 `--backend codex`）：
...
调用 Codex 复用会话进行后端架构规划。**必须使用以下显式命令模板**（禁止省略 `--backend codex`）：

```bash
{{CCG_BIN}} --backend codex {{LITE_MODE_FLAG}} resume <CODEX_SESSION> - "{{WORKDIR}}" <<'EOF'
ROLE_FILE: .ccg/prompts/codex/architect.md
<TASK>
需求：<用户选择的方案>
上下文：<阶段 2 的分析结果>
</TASK>
...
调用 Codex 复用会话进行后端代码审查。**必须使用以下显式命令模板**（禁止省略 `--backend codex`）：

```bash
{{CCG_BIN}} --backend codex {{LITE_MODE_FLAG}} resume <CODEX_SESSION> - "{{WORKDIR}}" <<'EOF'
ROLE_FILE: .ccg/prompts/codex/reviewer.md
<TASK>
需求：审查以下后端代码变更
上下文：<git diff 或代码内容>
</TASK>
