...
> **📦 Level 1 门禁已封装为 Skill**
>
> Level 1 智能路由逻辑已封装到 `level1-gate` Skill 中，通过 `inject_skills` 机制自动注入到所有 CCG 命令。
>
> **详细说明**：查看 [skills/level1-gate/SKILL.md](./skills/level1-gate/SKILL.md)

#### 路径 A：纯自然语言

由 `level1-gate` Skill 自动执行以下步骤：

**步骤 1**：调用 `mcp______enhance` 增强用户需求（降级链：`mcp__ace-tool__enhance_prompt` → Claude 自增强）
...
**命令-代理映射**：保持现有 27 个命令的映射关系不变，具体见下方任务类型映射表
...
**代理职责边界**：
- ✅ 执行自己的工作流（如 6 阶段工作流）
- ✅ 调用工具完成任务（Read/Edit/Write/Bash/MCP）
- ✅ 通过 `mcp______zhi` 确认关键决策
- ✅ 工作流结束后提示后续可选命令（如"您可以执行 /ccg:commit"）
- ✅ 链式工作流中调用下一阶段命令（如 spec/team 系列）
- ❌ 推荐**替代命令**（如"建议使用更轻量的命令"、"建议降级为 xxx"）
- ❌ 重复执行 Level 1 的命令推荐流程

**违规检测**：
- 若代理在执行过程中推荐替代命令，视为违规
- 违规时：立即停止，通过 `mcp______zhi` 报告违规行为，等待用户指示
...
> **P1 迁移状态（2026-02-19）**：以下 7 个命令已完成 Skill 化迁移，通过 `.ccg/config.toml` 的 `[migration.p1]` 区块控制灰度开关。
> - `analyze` [Skill] -- `skills/analyze/SKILL.md`
> - `plan` [Skill] -- `skills/plan/SKILL.md`
> - `enhance` [Skill] -- `skills/enhance-prompt/SKILL.md`
> - `init` [Skill] -- `skills/init-project/SKILL.md`
> - `clean-branches` [Skill] -- `skills/clean-branches/SKILL.md`
> - `worktree` [Skill] -- `skills/worktree/SKILL.md`
> - `rollback` [Skill] -- `skills/rollback/SKILL.md`
> 一键回退：将 `config.toml` 中对应命令值改回 `"agent"` 即可。
...
| `{{BACKEND_FLAG}}` | `--backend <primary> ` 或空字符串 | `.ccg/config.toml` 中的 `[routing.<command>].primary` 配置，有值时生成 `--backend <primary> `（注意尾随空格），无配置时为空 |
...
**GitHub MCP 已集成 26 个工具，优先使用 MCP 工具而非 `gh` CLI。**
