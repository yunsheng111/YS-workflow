...
```bash
# 读取 CCG_BIN 路径（默认：~/.claude/bin/codeagent-wrapper.exe）
CCG_BIN="{{CCG_BIN}}"
WORKDIR="{{WORKDIR}}"

# 检查 LITE_MODE（如果为 true，跳过外部模型调用）
...
echo 'ROLE_FILE: ~/.claude/.ccg/prompts/codex/analyzer.md
<TASK>
需求：{{YOUR_TASK_HERE}}
</TASK>
OUTPUT: structured-analysis' | \
"$CCG_BIN" --backend codex - "$WORKDIR" 2>&1 > /tmp/codex-output.txt &
...
```bash
# 使用 Bash 工具，run_in_background: true
echo 'ROLE_FILE: ~/.claude/.ccg/prompts/gemini/analyzer.md
<TASK>
需求：{{YOUR_TASK_HERE}}
</TASK>
OUTPUT: structured-analysis' | \
"$CCG_BIN" --backend gemini --gemini-model "$GEMINI_MODEL" - "$WORKDIR" 2>&1 > /tmp/gemini-output.txt &

GEMINI_PID=$!
```

#### 步骤 4：等待结果并提取 SESSION_ID

```bash
# 等待 Codex 完成（最多 10 分钟）
wait $CODEX_PID || echo "Codex 超时或失败"

# 等待 Gemini 完成（最多 10 分钟）
wait $GEMINI_PID || echo "Gemini 超时或失败"

# 提取 SESSION_ID
CODEX_SESSION=$(grep -oP 'SESSION_ID:\s*\K[a-f0-9-]+' /tmp/codex-output.txt || echo "")
GEMINI_SESSION=$(grep -oP 'SESSION_ID:\s*\K[a-f0-9-]+' /tmp/gemini-output.txt || echo "")

# 输出结果
echo "Codex SESSION_ID: $CODEX_SESSION"
echo "Gemini SESSION_ID: $GEMINI_SESSION"
```
