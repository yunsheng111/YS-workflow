...
> **判定**：纯工具代理。调试分析通过代码检索、日志分析和浏览器工具完成，不调用任何外部模型（Codex/Gemini）。
> - 文件中无 `{{CCG_BIN}}`、`--backend codex/gemini`、`TaskOutput`、`ROLE_FILE`

> **预留**：未来如需引入多模型调试能力，可参考 `.doc/standards-agent/dual-model-orchestration.md` 添加双模型调用规范。
...
2. 获取错误日志、堆栈跟踪、复现步骤
3. 确认错误环境：开发/测试/生产、OS、运行时版本
4. 确认错误频率：必现 / 偶现 / 特定条件触发

### 阶段 2：上下文检索

1. 调用 `mcp__ace-tool__search_context` 检索错误相关代码
2. 追踪调用链：从错误点回溯至入口
3. 检查最近变更：`git log` 定位可疑提交

### 阶段 3：生成假设列表

1. 根据错误现象和代码分析，列出所有可能的根因假设
2. 按可能性从高到低排序
3. 每个假设明确：假设内容、验证方法、预期结果
4. 必要时调用 `mcp______context7` 检查框架版本已知 Bug
5. UI 相关问题时调用 `mcp______uiux_suggest` 判断是否为设计层面的问题

### 阶段 4：逐一验证假设

1. 对每个假设设计验证实验
2. 使用日志注入、断点调试、条件复现等手段
3. 前端问题使用 Chrome DevTools MCP 进行运行时分析
4. 记录验证结果：确认 / 排除 / 需要更多信息

### 阶段 5：确定根因

1. 确认唯一根因或多因素组合
2. 搜索类似问题的已知解决方案
3. 调用 `mcp______zhi` 展示根因分析结论供用户确认
...
2. 评估修复影响范围和回归风险
3. 建议相应的测试用例
4. 调用 `mcp______zhi` 展示修复方案并确认实施
5. 修复完成后调用 `mcp______ji` 存储缺陷模式，更新项目已知问题库

### 阶段 7：创建 GitHub Issue（可选）

1. 修复完成后，调用 `mcp______zhi` 询问用户是否创建 Bug Issue
2. 如果用户选择创建：
