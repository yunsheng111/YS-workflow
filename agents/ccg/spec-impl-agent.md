...
### 阶段 1：计划加载
1. 读取 `.claude/spec/plans/` 下的最新计划文件
2. 读取关联的约束集
...
10. 审计发现 Minor 问题 → 记录并继续，在最终报告中汇总
...
### 阶段 5：完成报告
13. 所有步骤执行完毕后，生成实施报告
14. 将报告写入 `.claude/spec/reviews/` 目录（供 spec-review 使用）
...
- 多模型审计调用必须并行执行
- 实施报告写入 `.claude/spec/reviews/` 供下游代理使用
