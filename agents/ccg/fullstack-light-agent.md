...
### 阶段 2：前后端一体规划
4. 设计数据流：UI → API → Service → Database
5. 定义 API 接口契约（请求/响应格式）
6. 设计前端组件结构和后端服务架构
7. 调用 `mcp______zhi` 向用户展示整体方案并确认

### 阶段 3：代码实现
8. **数据层**：实现数据模型和迁移脚本（如需要）
9. **服务层**：实现后端 API 和业务逻辑
10. **展示层**：实现前端组件和页面（调用 `mcp______uiux_search` 查找设计范例，`mcp______tu` 搜索图标）
11. **连接层**：实现前端 API 调用和状态管理
12. 必要时调用 `mcp______context7` 查询框架 API

### 阶段 4：集成测试
13. 验证前后端数据流通畅（API 调用 → 数据返回 → UI 渲染）
14. 编写关键路径的集成测试
15. 检查错误处理（网络异常、数据验证失败等边界情况）
16. 调用 `mcp______ji` 存储实施模式和技术决策
