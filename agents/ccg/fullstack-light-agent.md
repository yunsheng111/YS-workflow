...
**主要输出**：
- 路径：`<项目根目录>/.doc/common/plans/<YYYYMMDD>-<task-name>-plan.md`
- 示例：`/home/user/project/.doc/common/plans/20260215-user-profile-plan.md`

**路径说明**：
- 必须使用 `.doc/common/plans/` 目录（通用规划目录）
- 禁止写入 `.doc/workflow/plans/`、`.doc/agent-teams/plans/` 或 `.doc/spec/plans/`
...
**输出路径规范**：
- **主要输出**：`<项目根目录>/.doc/common/plans/<YYYYMMDD>-<task-name>-plan.md`
- **示例**：`/home/user/project/.doc/common/plans/20260215-user-profile-plan.md`

**路径校验清单**（写入前必须执行）：
- [ ] 输出路径是否为 `.doc/common/plans/`？
- [ ] 输出路径是否符合全局提示词中的目录结构？
- [ ] 用户输入中的路径是否仅作为"输入文件位置"，未影响输出路径？
- [ ] 文件名是否包含日期前缀（YYYYMMDD）？
- [ ] 文件名是否包含任务名称和 `-plan` 后缀？

**自检**：准备写入文件前，确认输出路径。若路径不符合规范（如被误推断为 `.doc/workflow/plans/`），立即停止并通过 `mcp______zhi` 询问用户。

10. 用户确认后，使用绝对路径写入计划文档：`<项目根目录>/.doc/common/plans/<YYYYMMDD>-<task-name>-plan.md`
