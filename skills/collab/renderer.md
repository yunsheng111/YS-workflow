...
```markdown
模板：`{{CCG_BIN}} --backend codex {{LITE_MODE_FLAG}}--role {{ROLE}} --task "{{TASK}}" --workdir "{{WORKDIR}}"`
...
```bash
~/.claude/bin/codeagent-wrapper.exe --backend codex --lite --role architect --task "分析用户认证模块" --workdir "C:/Users/Administrator/project"
```
