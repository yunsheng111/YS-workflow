...
CCG 命令层实现智能路由，根据任务特征自动选择最优执行方式：

```
任务复杂度评估
  ├─ 低复杂度 → 主代理直接执行 (1x)
  │   └─ 示例：commit, enhance, rollback, clean-branches
  │
  ├─ 中等复杂度 → Subagent 执行 (1x)
  │   ├─ 无协作需求 → 单 Subagent
  │   │   └─ 示例：analyze, plan, review, test
  │   └─ 需要多视角 → 多 Subagent 并行
  │       └─ 示例：spec-research, spec-plan
  │
  └─ 高复杂度 → 根据协作需求选择
      ├─ 无需实时协作 → Subagent 串行 (1x)
      │   └─ 示例：frontend, backend, execute
      │
      ├─ 需要阶段协作 → 混合策略 (2-3x)
      │   └─ 示例：workflow (侦察→规划→实施→审查)
      │
      └─ 需要实时协作 → Agent Teams (7x)
          ├─ 文件数量 > 8
          ├─ 需要接口契约实时对齐
          └─ 示例：team-exec
```
...
每个命令文件包含路由元数据（YAML frontmatter）：

```yaml
routing:
  complexity: low | medium | high
  collaboration: low | medium | high
  default_mode: main | subagent | agent_teams | mixed
  cost_estimate: "1x" | "2-3x" | "7x"
  file_count_threshold: 0-10
  requires_realtime_alignment: true | false
```
...
方案 1：使用 Subagents 串行执行
  ...
  权衡考虑：
    ...
    - 需要主代理协调
    - 成本降低到 1x
  实施方式：主代理按 Layer 顺序调用 Subagents，手动传递数据
