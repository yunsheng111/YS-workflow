...
task: "CCG 架构优化 - 任务规划层职责分配与命令层必要性评估"
---
...
**描述**：Subagents 和 Agent Teams 的通信模型在技术层面不可互换。
...
**违反后果**：
- 使用 Subagents 处理需要实时协作的任务会导致通信失败
- 尝试让 Subagents 直接通信会导致系统错误
- 架构设计违反技术限制，无法正常运行
...
**描述**：Subagents 和 Agent Teams 的进程模型根本不同，影响状态管理和资源回收。
...
**违反后果**：
- 使用 Subagents 处理需要状态持久化的任务会导致状态丢失
- Agent Teams 未正确清理会导致资源泄漏和成本持续增加
- 混用两种模式会导致不可预测的行为
...
**描述**：Agent Teams 的 Token 成本约为 Subagents 的 7 倍。
...
**违反后果**：
- 过度使用 Agent Teams 导致运营成本失控
- 用户预算超支，影响系统可持续性
- 简单任务使用 Agent Teams 造成资源浪费

---

### HC-4: 嵌套约束（技术限制）

**描述**：Subagents 和 Agent Teams 都有嵌套限制。
...
**描述**：Subagents 和 Agent Teams 的权限控制粒度不同。
...
**描述**：Subagents 对用户透明，Agent Teams 需用户主动管理。
...
**权衡考虑**：
- 如果完全去除抽象层，可提供详细的决策树和交互式向导
- 如果用户是高级开发者，可提供直接访问底层 API 的选项
- 保持命令层作为默认入口，同时支持高级用户绕过

---

### SC-2: 任务规划职责分配（架构决策）

**描述**：任务规划应由谁负责 - 主代理、Subagents 还是 Agent Teams Lead？
...
**权衡考虑**：
- 如果主代理规划导致上下文膨胀，可降级到 Subagent 规划
- 如果 Subagent 规划结果需要大量回传，可升级到主代理规划
- 根据任务复杂度和成本预算动态选择规划者
...
**权衡考虑**：
- 如果去除命令层，需要在主代理提示词中嵌入完整的决策树
- 如果保留命令层，需要维护命令文档和代理映射关系
- 可以采用混合方案：保留核心命令，允许高级用户直接调用代理

---

### SC-4: 混合策略优化（成本与质量平衡）

**描述**：在不同阶段使用不同模式，平衡成本与质量。
...
**权衡考虑**：
- 如果成本预算紧张，可全程使用 Subagents
- 如果质量要求极高，可全程使用 Agent Teams
- 混合策略是大多数场景的最优解，需要标准化阶段间数据传递
...
**描述**：如果去除命令层，用户需要直接理解 Subagents vs Agent Teams 的差异。
...
**描述**：如果任务规划职责分配不清晰，可能导致主代理和 Subagents/Agent Teams 重复规划。
...
**描述**：过度使用 Agent Teams 导致 Token 成本失控。
...
**描述**：如果尝试在 Subagent 内启动 Subagent 或嵌套 Agent Teams，系统可能崩溃。
