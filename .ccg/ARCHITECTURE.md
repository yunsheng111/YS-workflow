...
**执行方式说明**：
- **Task 调用**（13 个）：使用 `Task(subagent_type="xxx")` 启动子代理，独立上下文执行
- **外部模型**（4 个）：通过 `codeagent-wrapper` 调用 Codex/Gemini，主代理整合结果
- **直接执行**（5 个）：主代理直接完成，无需子代理或外部模型
