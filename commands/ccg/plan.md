...
**代理**：`planner`（`agents/ccg/planner.md`）
...
- **仅规划**：本命令允许读取上下文与写入 `.doc/common/plans/*` 计划文件，但**禁止修改产品代码**
...
4. 生成实施计划并保存到 `.doc/common/plans/<feature-name>.md`
...
planner 代理会自动将计划保存至 `.doc/common/plans/<feature-name>.md`，主代理无需额外处理。
